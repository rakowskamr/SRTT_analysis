%% SRTT - reaction time for left and right hand separately

% This script gets the reaction time per block of the SRTT for 
...
for pSWS = 1:l_SWS
    ...
    results_pre_blocks_A_RT = results_pre_blocks_A(:,3);
    ...
    Lt_results_pre_blocks_A_RT(i,2) = mean(L_results_pre_blocks_A);
    ...
    Rt_results_pre_blocks_A_RT(i,2) = mean(R_results_pre_blocks_A);
    end
    
    for i = 1: b_B % get RT from all blocks that have seq 
    results_pre_blocks_B = results_blocks_B(i).exp_data;
    results_pre_blocks_B_RT = results_pre_blocks_B(:,3);
    ...
    Lt_results_pre_blocks_B_RT(i,2) = mean(L_results_pre_blocks_B);
    ...
    Lt_results_pre_blocks_randomA_RT(i,2) = mean(L_results_pre_blocks_randomA);
    ...
    Rt_results_pre_blocks_randomA_RT(i,2) = mean(R_results_pre_blocks_randomA);
    end

    for i = 1: b_randomB % get RT from all blocks that have random A
    results_pre_blocks_randomB = results_blocks_randomB(i).exp_data;
    results_pre_blocks_randomB_RT = results_pre_blocks_randomB(:,3);
    ...
    Lt_results_pre_blocks_randomB_RT(i,2) = mean(L_results_pre_blocks_randomB);
    ...
    Rt_results_pre_blocks_randomB_RT(i,2) = mean(R_results_pre_blocks_randomB);
    end
...
filesave = sprintf('R_SWS_right_blocks_LEARN2.mat'); % <------- CHANGE THE NAME
filesave = fullfile(outputFolder_LeftRight,filesave);
save(filesave,'R_SWS_right_results');

filesave = sprintf('NR_SWS_right_blocks_LEARN2.mat');% <------- CHANGE THE NAME
filesave = fullfile(outputFolder_LeftRight,filesave);
save(filesave,'NR_SWS_right_results');

filesave = sprintf('R_SWS_left_blocks_LEARN2.mat');% <------- CHANGE THE NAME
filesave = fullfile(outputFolder_LeftRight,filesave);
save(filesave,'R_SWS_left_results');

filesave = sprintf('NR_SWS_left_blocks_LEARN2.mat');% <------- CHANGE THE NAME
filesave = fullfile(outputFolder_LeftRight,filesave);
